...
api.patch('/pizzas/:name', (req, res, next) => {
    ...
    if (teamname === undefined || suggestionName === undefined || mode === undefined) {
        res.json(403, { error: 'Bad request: mode, teamname or name of pizza is not defined' });
        ...
        try {
            ...
            res.json(200, { msg: 'OK: Suggestion is revoted' });
        } catch (error) {
            res.json(403, { error: 'Bad request: there is no such suggestion for the team' });
        }
    } else {
        res.json(403, { error: 'Bad request: mode value is not supported' });
    }
});
